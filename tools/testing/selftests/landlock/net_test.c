...
{
	struct service_fixture srv0;
	/* srv1 has a rule with no access but quiet bit set. */
...
TEST_F(audit, connect_quiet_bind)
{
	const char *audit_evt = (variant->prot.type == SOCK_STREAM ?
					 "net\\.connect_tcp" :
					 "net\\.connect_udp");
	const int access_bind =
		(variant->prot.type == SOCK_STREAM ?
			 LANDLOCK_ACCESS_NET_BIND_TCP :
			 LANDLOCK_ACCESS_NET_BIND_UDP);
	const int access_connect =
		(variant->prot.type == SOCK_STREAM ?
			 LANDLOCK_ACCESS_NET_CONNECT_TCP :
			 LANDLOCK_ACCESS_NET_CONNECT_UDP);
	const struct landlock_ruleset_attr ruleset_attr = {
		.handled_access_net = access_bind | access_connect,
		.quiet_access_net = access_bind,
	};
	const struct landlock_ruleset_attr ruleset_attr_2 = {
		.handled_access_net = access_bind | access_connect,
		.quiet_access_net = access_connect,
	};
	const struct landlock_net_port_attr quiet_rule = {
		.allowed_access = 0,
		.port = self->srv1.port,
	...
	EXPECT_EQ(-EACCES, connect_variant(sock_fd, &self->srv1));
	EXPECT_EQ(0,
		  matches_log_prot(self->audit_fd, audit_evt, "daddr",
...
TEST_F(audit, sendmsg)
{
	...
	EXPECT_EQ(0, matches_log_prot(self->audit_fd, "net.sendto_udp", "daddr",
				      NULL, "dest", self->unspec_srv0.port));
