...
// clang-format off
FIXTURE_VARIANT_ADD(scoped_audit, quiet_pathname_socket)
{
	// clang-format on
	.abstract_socket = false,
	.quiet_scoped = LANDLOCK_SCOPE_PATHNAME_UNIX_SOCKET,
};
...
TEST_F(scoped_audit, connect_to_child)
{
	...
	int ruleset_fd;
	const __u64 scoped = variant->abstract_socket ?
	...
	bool should_audit = !(variant->quiet_scoped & scoped);
