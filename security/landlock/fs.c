			    ...
			    access_mask_t access_rights, const int flags)
{
	...
	if (!err && (flags & LANDLOCK_ADD_RULE_NO_INHERIT)) {
		...
		path_get(&walker);
		while (true) {
			const struct landlock_rule *ancestor_rule;
			...
			mutable_rule = (struct landlock_rule *)ancestor_rule;
			if (WARN_ON_ONCE(!mutable_rule || mutable_rule->num_layers != 1)) {
				err = -EINVAL;
				break;
			}
	...
	struct dentry *const dentry_child2)
{
	...
	struct landlock_request dummy_request_parent2 = {};
	struct landlock_request *const log_request_parent2_ =
		log_request_parent2 ? log_request_parent2 : &dummy_request_parent2;
	...
	struct collected_rule_flags *rule_flags_parent2 =
		&log_request_parent2_->rule_flags;
			       ...
			       layer_mask_t *const override_layers)
{
	...
	while (true) {
		...
		rule = find_rule(domain, walker.dentry);
		if (rule) {
