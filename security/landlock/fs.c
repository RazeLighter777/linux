			   ...
			   struct dentry *const dentry_child2)
{
	...
	while (true) {
		...
		allowed_parent1 =
			allowed_parent1 ||
			landlock_unmask_layers(rule, layer_masks_parent1, rule_flags_parent1);
		allowed_parent2 =
			allowed_parent2 ||
			landlock_unmask_layers(rule, layer_masks_parent2, rule_flags_parent2);
 ...
 *
 * This helper is useful to begin a path walk from the @dir directory to the
 * mount root used as a mount point.  This mount point is the common
 ...
 * - false if the walk reached the mount root.
 */
				    ...
				    struct collected_rule_flags *const rule_flags)
{
	...
	while (true) {
		...
		if (landlock_unmask_layers(find_rule(domain, walker.dentry),
					   layer_masks_dom,
					   rule_flags)) {
			/*
		 ...
		 * directory.
		 */
		if (walker.dentry == dir->mnt->mnt_root)
			break;
					   ...
					   struct dentry *const dcache_entry)
{
	...
	for (layer_index = 0; layer_index < rule->num_layers; layer_index++) {
		...
		u16 layer_bit = BIT_ULL(layer->level - 1);
