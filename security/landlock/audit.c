 ...
 */
static size_t
get_layer_from_fs_deny_masks(access_mask_t *const access_request,
			     ...
			     u8 quiet_optional_accesses, bool *quiet)
{
	...
	if (access_req & LANDLOCK_ACCESS_FS_TRUNCATE) {
		...
		should_quiet = !!(quiet_optional_accesses & BIT(0));
	}
	if (access_req & LANDLOCK_ACCESS_FS_IOCTL_DEV) {
		...
		if (layer > youngest_layer) {
			...
			should_quiet = !!(quiet_optional_accesses & BIT(1));
		} else if (layer == youngest_layer) {
			missing |= LANDLOCK_ACCESS_FS_IOCTL_DEV;
			if (access_req & LANDLOCK_ACCESS_FS_TRUNCATE) {
				WARN_ON_ONCE(should_quiet !=
		...
		/* If layer < youngest_layer, don't add IOCTL_DEV to missing */
	}
...
static void test_get_layer_from_fs_deny_masks(struct kunit *const test)
{
	...
	KUNIT_EXPECT_EQ(test, 0,
			get_layer_from_fs_deny_masks(
				&access, deny_mask, quiet_optional_accesses, &quiet));
			 ...
			 const struct landlock_request *const request)
{
	...
	if (missing) {
		...
		if (request->masks) {
			...
			object_quiet_flag =
				!!(request->rule_flags.quiet_masks.access[youngest_layer]);
		} else {
