...
/*
 * security_named_unix_socket_connect() - Check if a named AF_UNIX socket can connect
 ...
 * Return: Returns 0 if permission is granted.
 */
int security_named_unix_socket_connect(const struct path *path)
{
	return call_int_hook(named_unix_socket_connect, path);
}
EXPORT_SYMBOL(security_named_unix_socket_connect);
